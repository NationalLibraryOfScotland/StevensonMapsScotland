...
// the Stevenson GeoJSON file for all the maps
	...
	var stevenson_geojson = 'https://geo.nls.uk/maps/stevenson/scripts/stevenson_areas_1_july.js';
...
console.log("filterinprocess = true");
...
// map.getLayers().getArray()[3] for dark blue selectable features
		...
		if ((parseInt(zoomval)  == 1) && (mapzoom < 9))
			...
			document.getElementById('zoom_statement').innerHTML = "Items with <strong>large extents</strong> shown boldly and selectable on map. Zoom in to view items with smaller extents.";
			...
			document.getElementById('zoom_statement').innerHTML = "Items with <strong>medium extents</strong> shown boldly and selectable on map. Zoom in or out to view items with smaller or larger extents.";
			...
			document.getElementById('zoom_statement').innerHTML = "Items with <strong>small extents</strong> shown boldly and selectable on map. Zoom out to view items with larger extents.";
			...
			document.getElementById('zoom_statement').innerHTML = "Items with <strong>small, medium and large extents</strong> shown boldly and selectable on map."
...
// function to unselect previous features and return the results and resultsheader divs back to original empty state
	...
	function deselect()
		...
		setResults(''); 
		setResultsheader('<p>No items selected - please click on a <strong>dark blue feature</strong> where you are interested in on the map to view details of items</p>');
		updateUrl();
	}
...
// set header depending on selectedFeatures.length
			...
			if (selectedFeatures.length < 1)
				resultsheader += '';
		            ...
		            resultsheader += '<div id="deselect"><a href="javascript:deselect();">Deselect these items</a></div><br/><p><strong>Results - 1 item:</strong><br />(hover mouse over each item to view extents).</p>';
			else if (selectedFeatures.length > 1)
	
		        resultsheader += '<div id="deselect"><a href="javascript:deselect();">Deselect these items</a></div><br/><p><strong>Results - ' + selectedFeatures.length + ' items:</strong><br />(hover mouse over each item to view extents).</p>';
		        ...
		        setResultsheader(resultsheader);
				  ...
				  if (selectedFeatures.length > 0) {
					...
					setResults(results);
				        ...
				        {setResults('No items selected - please click on a <strong>dark blue feature</strong> where you are interested in on the map to view details of items');}
				  }
...
// function to highlight items in the Results div and select / zoom to selected feature extent
...
// function to unselect items from the resultslist div when the mouse leaves it
...
// function to display information on numbers of maps covering mouse cursor
	...
	function displayFeatureInfoOnHover(pixel) {	
		 ...
		 var feature = map.forEachFeatureAtPixel(pixel, function(feature) {
			...
			document.getElementById('hover_results').innerHTML = selectedFeatures.length + " item with a <strong>" + zoomvalextent + " extent</strong> covers mouse location."; 
			else if (selectedFeatures.length > 1)

			document.getElementById('hover_results').innerHTML = selectedFeatures.length + " items with <strong>" + zoomvalextent + " extents</strong> cover mouse location."; 
...
// alter headers
			...
			setResultsheader('<p>Selecting items... please wait</p>');
			setResults(''); 
...
// populates the resultsheader div on the right with default text 
	...
	//    if (!str) str = '<p>No items selected - please click on where you are interested in on the map to the left to view details of items</p>';
	    document.getElementById('resultsheader').innerHTML = str;
	}

// resultsheader += '<p>No items selected - please click on where you are interested in on the map to the left to view details of items</p>';
...
// listener on changes on map.getLayers().getArray()[3] and when features are present, change 'loading_results' div
		  ...
		  if (map.getLayers().getArray()[3].getSource().getFeatures().length > 0) {
			document.getElementById('loading_results').innerHTML  = "";
                ...
                console.log("Source state once data is loaded: ", stevenson_geojson_layer.getSource().getState());
	    });
...
// listener on map.getLayers().getArray()[1] - when features are loaded from GeoJSON initiate filter()
		...
		if (stevenson_geojson_layer_ready)
